...
def generate_dynamic_test_data():
    ...
    for _ in range(num_entries // 10):  # Splitting the number of entries into 10% for shuffles
        ...
        shuffle = Shuffle.objects.create(
            concept=concept,
            status=random.choice([status[0] for status in Shuffle.ShuffleStatus.choices])
        )
...
# Call the function to generate dynamic test data
test_data = generate_dynamic_test_data()
